...
class Add(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(1, n):
            out = out + x[i]
        return out
class Add_BN(nn.Module):
    def __init__(self, c2):
        super(Add_BN, self).__init__()
    ...
    def forward(self, x):
        ...
        out = self.bn(out)
        return out
class Add_BN_Atten(nn.Module):
    def __init__(self, c2):
        super(Add_BN_Atten, self).__init__()
        self.bn = nn.BatchNorm2d(c2)
    ...
    def forward(self, x):
        ...
        for i in range(1, n):
            out = out + x[i]
        temp = out
        out = self.sigmoid(self.bn(temp))
        out = temp.mul(out)
        return out
class Add_Atten(nn.Module):
    def __init__(self):
        super(Add_Atten, self).__init__()
        # self.bn = nn.BatchNorm2d(c2)
        self.sigmoid = nn.Sigmoid()
        # self.d = dimension
    def forward(self, x):
...
class Add_Attention(nn.Module):
    def __init__(self, c1):
        ...
        # self.bn = nn.BatchNorm2d(c2)
        self.conv = nn.Conv2d(in_channels=c1, out_channels=c1, kernel_size=1, stride=1,
    ...
    def forward(self, x):
        ...
        out = x[0]
        for i in range(1, n):
            out = self.conv(out) + self.conv(x[i])
        temp = out
        # out = self.relu(out)
        out = self.mish(out)
        out = self.conv(out)
...
class lighten_layer(nn.Module):
    def __init__(self, c1):
        super().__init__()
        self.dim = int(c1/3)
        self.dim2 = c1 - self.dim
        self.partial_conv3 = nn.Conv2d(self.dim, self.dim, 3, 1, 1, bias=False)
        self.point_wise_conv = nn.Conv2d(in_channels=c1, out_channels=c1, kernel_size=1, stride=1,
                                         padding=0, groups=1, bias=False)

    def forward(self, x):
        tensor = []
        temp = F.interpolate(x, size=(20, 20))
        bs, c, h, w = temp.shape
        for b in range(0, bs):
            tensor1 = []
            for i in range(0, c):
                # 得到特征图u尺寸：h*w
                u = temp[b, i, :, :].squeeze().cpu().detach().numpy()
                # 对其中一个维度进行均值压缩，以降低计算量1*w
                # v = np.mean(u, axis=1)
                uT = u.T  # w*h
                # uT = uT.detach().numpy()
                D = np.cov(uT)  # h*h
                try:
                    invD = np.linalg.inv(D)  # # h*h
                    a1 = []
        ...
        if len(tensor.shape) == 3:
            ...
            for b1 in range(bs):
                ...
                # 已知各个特征层的角标（shape：bs*c），将他们取出组成一个新的特征层，输入到部分卷积中。
                # 已知每个bs组内的下标
                # entt1[i].sort(reverse=True)
                max_number = sorted(entt, reverse=True)[:self.dim]
                other_number = sorted(entt, reverse=True)[self.dim:]
                for t in max_number:
                    index1 = entt.index(t)
                    a.append(x[b1, :, :, :][index1, :, :].unsqueeze(0).unsqueeze(0))
                a = torch.cat(a, dim=1)
                # max_index1.append(index1)
                a_.append(a)
                for tt in other_number:
                    index2 = entt.index(tt)
                    b.append(x[b1, :, :, :][index2, :, :].unsqueeze(0).unsqueeze(0))
                b = torch.cat(b, dim=1)
                # max_index2.append(index2)
                b_.append(b)
                # max_index1_.append(max_index1)
                # max_index2_.append(max_index2)
            x_raw_new = torch.cat(a_, dim=0).to(torch.float32)
            x_raw_other = torch.cat(b_, dim=0).to(torch.float32)
            x_raw_new = self.partial_conv3(x_raw_new)
            x = torch.cat((x_raw_new, x_raw_other), dim=1)
            x = self.point_wise_conv(x)
        else:
            x1, x2 = torch.split(x, [self.dim, self.dim2], dim=1)
